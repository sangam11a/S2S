...
#ifndef USER_DEFINED_FOLDER_FLASH_MEMORY_MT25QL_H_
#define USER_DEFINED_FOLDER_FLASH_MEMORY_MT25QL_H_
...
class mt25ql : public DEVICE_ID, public Flag_Status_Register
{
public:
typedef enum{
	SPI_READ,
...
public:
	...
	void CS_HIGH(GPIO_TypeDef *GPIO_PORT, uint16_t GPIO_PIN);

	void SPI_Operation(SPI_HandleTypeDef *SPI, uint8_t cmd, uint32_t address, GPIO_TypeDef *GPIO_PORT, uint16_t GPIO_PIN, SPI_OPERATIONS spi_operations, uint8_t *data_buffer, uint8_t =5);
//	void SPI_Write();
//	void SPI_ReadWrite();
	void Read_ID(SPI_HandleTypeDef *SPI) ;
...
//			uint16_t size);
	void Page_Write(SPI_HandleTypeDef *SPI, uint32_t address, uint8_t *data, uint8_t addressing_mode,uint16_t size = 5) ;
	...
	//void Subsector_Erase_32KB(uint32_t address);
	//void Subsector_Erase_32KB_4B(uint32_t address);
	//
	//void Subsector_Erase_4KB(uint32_t address);
	//void Subsector_Erase_4KB_4B(uint32_t address);
...
#endif /* USER_DEFINED_FOLDER_FLASH_MEMORY_MT25QL_H_ */
